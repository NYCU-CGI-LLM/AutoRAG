...
from app.schemas.pipeline_config import PipelineConfigModel
...
def extract_best_config(trial_path: str, output_path: Optional[str] = None) -> Dict:
	...
	raw_yaml = summary_df_to_yaml(trial_summary_df, config_dict)
	raw_yaml["vectordb"] = extract_vectordb_config(trial_path)
	try:
		pipeline_config = PipelineConfigModel.parse_obj(raw_yaml)
	except ValidationError as e:
		logger.error("Failed to parse pipeline config: %s", e)
		raise
	if output_path is not None:
		with open(output_path, "w") as f:
			yaml.safe_dump(pipeline_config.dict(), f)
...
class BaseRunner:
	def __init__(self, config: PipelineConfigModel, project_dir: Optional[str] = None):
		self.config = config
		...
		node_lines = deepcopy(config_dict["node_lines"])
	...
	def from_yaml(cls, yaml_path: str, project_dir: Optional[str] = None):
		...
		pipeline_config = PipelineConfigModel.parse_obj(raw_config)
		return cls(pipeline_config, project_dir=project_dir)
...
class Runner(BaseRunner):
	def run(self, query: str, result_column: str = "generated_texts", **run_kwargs):
		"""
		...
		:param run_kwargs: Additional keyword arguments to pass to the module.pure method.
