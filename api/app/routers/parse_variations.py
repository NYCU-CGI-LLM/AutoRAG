...
async def _get_kb_parse_variation_dir(kb_id: UUID, parse_id: UUID) -> str:
    # Specific directory for a single parsing variation under the 'variations' directory
    base_variations = await _get_kb_variations_dir(kb_id)
    return os.path.join(base_variations, str(parse_id))

async def _get_kb_parse_variation_metadata_path(kb_id: UUID, parse_id: UUID) -> str:
    return os.path.join(await _get_kb_parse_variation_dir(kb_id, parse_id), "parse_variation_metadata.json")

async def _read_parse_variation_metadata(kb_id: UUID, parse_id: UUID) -> Optional[ParsingVariation]:
    metadata_path = await _get_kb_parse_variation_metadata_path(kb_id, parse_id)
    ...
    try:
        ...
        print(f"Error reading or parsing parse_variation_metadata.json for PV {parse_id} in KB {kb_id}: {e}")
        return None
...
async def list_parse_variations(kb_id: UUID):
    kb_parsed_data_dir = await _get_kb_variations_dir(kb_id) # Base dir for all parse variations of this KB
...
@router.get("/{parse_id}", response_model=ParsingVariation)
async def get_parse_variation(kb_id: UUID, parse_id: UUID):
    metadata = await _read_parse_variation_metadata(kb_id, parse_id)
    if not metadata:
        # Check if KB or specific variation dir exists for more accurate error
        variation_dir = await _get_kb_parse_variation_dir(kb_id, parse_id)
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Metadata for parsing variation {parse_id} not found, but its directory exists.")
    return metadata

@router.delete("/{parse_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_parse_variation(kb_id: UUID, parse_id: UUID):
    variation_dir = await _get_kb_parse_variation_dir(kb_id, parse_id)
